...
task unit-test {
	...
	exec { & $opencover -register:user -target:$nunitConsole -targetargs:"$testsDlls" -filter:"+[DataStructures*]* +[DesignPatterns]*HardcoreSingleton -[*UnitTests]*" -output:..\coverage.xml }
}
